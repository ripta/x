...
export default class Firework {
    ...
    update() {
        if (this.phase === 0 && this.particles.every(particle => particle.isDone())) {
            this.phase = 1;
            ...
            if (burstType < 0.5) {
                ...
                for (let i = 0; i < burstCount; i++) {
                    ...
                    const burst = new Particle(this.p, startingPos, startingVel, this.gravity, this.hue, true);
    ...
    isDone(): boolean {
        return this.particles.length > 1 && this.particles.every(particle => particle.isDone());
    }
}